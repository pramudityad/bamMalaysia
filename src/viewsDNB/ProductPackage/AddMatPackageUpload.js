...
class AddMatPackageUpload extends React.Component {
  constructor(props) {
    ...
    this.exportFormatBundleMaterial =
      this.exportFormatBundleMaterial.bind(this);
  ...
  getPackageDataAPI() {
    ...
    let whereOr =
      '{"$or" : [{"product_name": ' +
      filter +
      '}, {"pp_id": "AdditionalMaterial"}, {"pp_cust_number": ' +
      filter +
      '}, {"physical_group": ' +
      filter +
      "}]}";
    ...
    this.getDatafromAPINODE(
      '/productpackage?q={"pp_id": "AdditionalMaterial"}&lmt=' +
        this.state.perPage +
        "&pg=" +
  ...
  getPackageDataAPI_all() {
    this.getDatafromAPINODE(
      '/productpackage?q={"pp_id": "AdditionalMaterial"}&noPg=1'
    ).then((res) => {
      if (res.data !== undefined) {
  ...
  saveProductPackageOneShot = async () => {
    ...
    if (isPackage === true) {
      const postPackage = await this.postDatatoAPINODE(
        "/productpackage/packageWithMaterial",
  ...
  exportFormatConfig = async () => {
    ...
    typeArray = typeArray.concat(
      dataPrint.map((pp) => pp.pp_id + " /// " + pp.product_name)
    );
  ...
  async saveAdditional() {
    ...
    for (let i = 0; i < addMaterial.length; i++) {
      dataUpload.push([
        ...
        addMaterial[i].material_type !== undefined &&
        addMaterial[i].material_type !== null &&
        addMaterial[i].material_type.length !== 0
          ? addMaterial[i].material_type
          : "Additional",
        addMaterial[i].material_origin,
        addMaterial[i].unit !== undefined &&
        addMaterial[i].unit !== null &&
        addMaterial[i].unit.length !== 0
          ? addMaterial[i].unit
          : "Additional",
        addMaterial[i].qty !== undefined &&
        addMaterial[i].qty !== null &&
        addMaterial[i].qty.length !== 0
          ? parseFloat(addMaterial[i].qty)
          : 1,
      ]);
    }
    let patchDataMat = await this.postDatatoAPINODE(
      "/productpackage/packageWithMaterial",
      { data: dataUpload }
    );
    if (
  ...
  AdditionalForm() {
    ...
    const addPP = this.state.product_package.filter(
              ...
              >
                <Card style={{ margin: "10px 10px 5px 10px" }}>
                  ...
                  <CardFooter>
                    ...
                    >
                      {" "}
                      <i className="fa fa-paste">&nbsp;&nbsp;</i> &nbsp;Form
                    </Button>
                  </CardFooter>
                </Card>
              </Collapse>
                  ...
                  <Col>
                    <div style={{ margin: "8px 0px" }}>
                      <small>
                        ...
                        {this.state.product_package[0] === undefined
                          ? 0
                          : this.state.product_package[0].materials.length}{" "}
                        data
                      </small>
                    </div>
                    <Pagination
                    ...
                    {this.state.additional_material.map((add, i) => (
                      <tr>
                        ...
                        <td>
                          <input
                            type="text"
                            name={"material_origin" + " /// " + i}
                            value={add.material_origin}
                            onChange={this.onChangeMaterialAdditional}
                            style={{ width: "100px" }}
                          />
                        </td>
