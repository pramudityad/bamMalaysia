...
import { Button, Card, CardBody, CardHeader, Col, InputGroup, InputGroupAddon, InputGroupText, Input, Row, Table, FormGroup, Label, Modal, ModalBody, ModalFooter, ModalHeader } from 'reactstrap';
...
class LoadingProcess extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      asp_list: [],
      no_filter_dsp : false,
      id_mr_request_change : null,
      change_mover_to : null,
    }
  ...
  getMRList() {
    ...
    filter_array.push('"current_mr_status":"JOINT CHECK FINISH"');
    ...
    if(this.state.no_filter_dsp === true){
      (filter_array.push('"$or" : [{"dsp_company_code" : {"$regex" : "' + this.state.shipment_detail.transporter + '", "$options" : "i"}}, {"dsp_company":{"$regex" : "' + this.state.shipment_detail.transporter_name + '", "$options" : "i"}}]'));
    }
  ...
  async proceedMilestone(e) {
    ...
    let res = await patchDatatoAPINODE('/matreq/loadingProcess/' + id_doc, this.state.tokenUser);
    if (res !== undefined) {
      if (res.data !== undefined) {
  ...
  async proceedShipment() {
    ...
    let res = await patchDatatoAPINODE('/matreq/loadingProcess', dataShipment, this.state.tokenUser);
    if (res !== undefined) {
      if (res.data !== undefined) {
        this.setState({ action_status: "success" }, () => {
          setTimeout(function () { window.location.reload(); }, 2000);
  ...
  handleChangeShipmentDetail(e) {
    ...
    if(name.toString() === "transporter"){
      ...
      this.setState({mr_data_selected : [], mr_checked : new Map() });
    }
  ...
  toggleChangeMover = (e) => {
    if (e !== undefined) {
      const id_doc = e.currentTarget.id;
      this.setState({ id_mr_request_change: id_doc, change_mover_to: null });
    } else {
      this.setState({ id_mr_request_change: null, change_mover_to: null });
    }
    this.setState((prevState) => ({
      modal_change_mover: !prevState.modal_change_mover,
    }));
  }

  handleChangeMover = (e) => {
    this.setState({change_mover_to : e.target.value});
  }

  saveChangeMover = async () => {
    const newMover = {
        "data": {
            "newMover": this.state.change_mover_to
        }
    }
    const resPatch = await patchDatatoAPINODE('/matreq/requestChangeMover/'+this.state.id_mr_request_change, newMover, this.state.tokenUser);
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col xs="12" lg="12">
            <Card>
              <CardHeader>
                Shipment Detail
                <Button onClick={this.handleCheckingForm} size="sm" style={{ float: 'right' }} color="success" disabled={this.state.mr_data_selected.length === 0 || this.state.shipment_detail.transporter === "All"}>
                      ...
                      <Col md="8">
                        <div style={{ display: 'flex', "align-items": "baseline" }}>
                          <Input type="select" placeholder="" name="transporter" onChange={this.handleChangeShipmentDetail} value={this.state.shipment_detail.transporter === undefined ? "" : this.state.shipment_detail.transporter}>
                            <option value="" disabled selected hidden>Select Transporter</option>
                            <option value={"All"}>{"All Transporter (Just For Show)"}</option>
                  ...
                  <tbody>
                    ...
                    {this.state.mr_list.map((list, i) =>
                      <tr key={list._id}>
                        ...
                        <Button outline color="warning" size="sm" style={{ width: "90px" }} id={list._id} onClick={this.toggleChangeMover} >
                          Change Mover
                        </Button>
                      </tr>
                    )}
        ...
        <Modal isOpen={this.state.modal_change_mover} toggle={this.toggleChangeMover} className={"modal-sm modal--box-input modal__delivery--ldm-approve"}>
          <ModalHeader>
            Request Change Mover
          </ModalHeader>
          <ModalBody>
            <FormGroup>
              <Label htmlFor="total_box">Change Delivery Company to</Label>
              <Input type="select" className="" placeholder="" onChange={this.handleChangeMover} name={this.state.change_mover_to} >
          ...
          <ModalFooter>
            <Button color="secondary" onClick={this.toggleChangeMover} style={{float : 'right', marginRight : '10px'}} size="sm" >
              Close
            </Button>
            <Button color="success" onClick={this.saveChangeMover} style={{float : 'right', marginRight : '10px'}} size="sm" >
              <i className="icon-check icons"></i> Change
            </Button>
          </ModalFooter>
        </Modal>
      </div>
