...
class PSUpload extends Component {
  ...
  getListTssrAll() {
    this.getDataFromAPINODE(
      '/plantspec?q={"id_mr_doc" : null, "submission_status" : "SUBMITTED", "site_info.site_id" : "' +
        this.state.data_mr.site_info[0].site_id +
        '" }'
    ).then((res) => {
  ...
  prepareView(dataTSSR) {
    ...
    let listSku = [];
    const dataTSSRMaterial = dataTSSR.packages.map((pp) =>
      pp.materials.map((material) => listSku.push(material.material_id))
    );
    let dataTechUniq = [];
    dataTSSR.packages.map((pp) =>
      dataTechUniq.find(
        (e) => e.id_tssr_boq_site_doc === pp.id_tssr_boq_site_doc
      ) === undefined
        ? dataTechUniq.push({
            id_tssr_boq_site_doc: pp.id_tssr_boq_site_doc,
            no_tssr_boq_site: pp.no_tssr_boq_site,
          })
        : null
    );
    ...
    if (site_mr_NE !== undefined) {
      site_NE = dataTSSR.site_info.find(
        (e) => e.site_id === site_mr_NE.site_id
      );
    }
    if (site_mr_FE !== undefined) {
      site_FE = dataTSSR.site_info.find(
        (e) => e.site_id === site_mr_FE.site_id
      );
    }
  ...
  async getPSAllocated() {
    ...
    const getDataMR = await this.getDatafromAPIBAM(
      '/mr_op?where={"id_tssr_doc" : "' + data_tssr_id + '"}'
    );
    if (getDataMR.data !== undefined) {
      const dataIDMR = getDataMR.data._items.map((e) => e._id);
      if (dataIDMR.length !== 0) {
        const getDataMR = await this.getDatafromAPIBAM(
          '/mr_pp_op?where={"id_mr_doc" : {"$in" : [' + dataIDMR + "]}}"
        );
        if (getDataMR.data !== undefined) {
        }
      }
  ...
  async connectMRtoTSSR() {
    ...
    dataPS.packages.forEach((pp) => {
      let wbsSite = null;
      const dataCDSite = selectedCD.find(
        (sc) => sc.id_tssr_boq_site_doc === pp.id_tssr_boq_site_doc
      );
    ...
    this.patchDatatoAPINODE(
      "/matreq/assignPlantSpecByTssr2/" +
        dataMRParent._id +
        "/ps/" +
        this.state.id_tssr_selected,
      dataTSSR
    ).then((res) => {
      if (res.data !== undefined) {
        this.patchDatatoAPINODE(
          "/matreq/requestMatreq/" + dataMRParent._id
        ).then((res) => {
  ...
  editQtyNE = (e) => {
    ...
    this.setState((prevState) => ({
      qty_ne: prevState.qty_ne.set(name, value),
    }));
  };
  ...
  editQtyFE = (e) => {
    ...
    const value = e.target.value;
    this.setState((prevState) => ({
      qty_fe: prevState.qty_fe.set(name, value),
    }));
  };
  ...
  handleChangeCDIDtoTSSR(e) {
    const name = e.target.name;
    const value = e.target.value;
    let dataCDSelected = this.state.list_cd_id_mr.find(
      (e) => e.id_cd_doc === value
    );
    let cd_id_to_tssr = this.state.cd_id_to_tssr;
    let indexCurrent = cd_id_to_tssr.findIndex(
      (e) => e.id_tssr_boq_site_doc === name
    );
  ...
  render() {
    ...
    return (
      <div>
            ...
            <Card>
              <CardHeader>
                ...
                <Button
                  color="success"
                  style={{ float: "right" }}
                  disabled={this.state.list_tssr_from_ps.length === 0}
              ...
              <CardBody>
                    ...
                    {this.state.data_mr !== null && (
                      <Fragment>
                        ...
                        <tr>
                          ...
                          >
                            SOW Type : {this.state.data_mr.sow_type}
                          </td>
                        </tr>
                        <tr>
                          <td
                ...
                <Fragment>
                                  ...
                                  this.state.data_mr.mr_type === "Return" ? (
                                    ...
                                    <Fragment>
                                      <tr>
                                        ...
                                        <td>:</td>
                                        <td>
                                          {this.state.data_tssr.siteList !==
                                          undefined
                                            ? this.state.data_tssr.siteList[0]
                                                .site_info.site_id
                                            : ""}
                                        </td>
                                      </tr>
                                ...
                                <tbody>
                                  <tr>
                                    ...
                                    <td style={{ paddingLeft: "10px" }}>
                                      {
                                        this.state.tssr_site_FE.site_info
                                          .site_id
                                      }
                                    </td>
                                  </tr>
                  ...
                  {this.state.data_mr !== null ? (
                    this.state.data_mr.sow_type === "RBS" &&
                    this.state.tssr_site_FE !== null ? (
                      <span style={{ color: "red" }}>
                        You just can choose Plant Spec with same SOW type
                      </span>
        ...
        {/* Modal Assign */}
        <Modal
          isOpen={this.state.modal_assign_ps}
        ...
        >
                  ...
                  {this.state.list_tssr_from_ps.map((tssr) => (
                    <tr>
                      ...
                      <td>
                        <Input
                          type="select"
                          name={tssr.id_tssr_boq_site_doc}
                          value={this.state.cd_id_to_tssr_selected.get(
                            tssr.id_tssr_boq_site_doc
                          )}
                          onChange={this.handleChangeCDIDtoTSSR}
                        >
                          <option></option>
          ...
          <ModalFooter>
            ...
            <Button
              ...
              style={{ float: "right" }}
              disabled={
                this.state.cd_id_to_tssr.length !==
                this.state.list_tssr_from_ps.length
              }
              onClick={this.connectMRtoTSSR}
            >
              Assign
            </Button>
          </ModalFooter>
