...
class MRList extends Component {
  ...
  async downloadMRlist() {
    ...
    let headerRow = ["MR ID", "MR MITT ID","MR Type","MR Delivery Type", "Project Name", "CD ID", "Tower ID", "Tower Name", "Current Status", "Current Milestone", "DSP", "ETA", "MR MITT Created On", "MR MITT Created By","Updated On", "Created On", "WP ID for WBS", "WBS HW for GI", "WBS HWAC (License)", "WBS LCM", "WBS PNRO", "WBS PNDO", "WBS HW Bulk", "WBS LCM Bulk for GI", "WBS PowHW Site Basis", "WBS PowLCM Site Basis", "WBS Kathrein"];
    ws.addRow(headerRow);
            ...
            <Card>
              <CardHeader>
                ...
                {((this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-Mover") === -1)) && (
                  ...
                  <Link to={'/mr-report'}>
                    <Button outline color="info" size="sm" style={{ float: 'right', marginRight : '10px' }}>MR Report</Button>
                  </Link>
                </React.Fragment>
                )}
              </CardHeader>
