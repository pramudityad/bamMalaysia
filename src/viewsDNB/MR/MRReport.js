...
import {convertDateFormatfull, convertDateFormat, convertDateFormatnewDate} from '../../helper/basicFunction';
import { getDatafromAPIEXEL, getDatafromAPINODE } from "../../helper/asyncFunction";
...
class MRList extends Component {
  ...
  getMRList() {
    ...
    this.state.filter_list[10] !== "" && (filter_array.push('"creator":{"$regex" : "' + this.state.filter_list[10] + '", "$options" : "i"}'));
    ...
    if((this.state.userRole.findIndex(e => e === "BAM-ASP") !== -1 || this.state.userRole.findIndex(e => e === "BAM-ASP Management") !== -1 || this.state.userRole.findIndex(e => e === "BAM-Mover") !== -1) && this.state.userRole.findIndex(e => e === "Admin") === -1){
      filter_array.push('"$or" : [{"dsp_company_code" : "'+this.state.vendor_code+'"}, {"dsp_company" : "'+this.state.vendor_name+'"}]');
    }
    filter_array.push('"$and" : [{"mr_delivery_type_code" : {"$ne" : "A1"}}, {"mr_delivery_type_code" : {"$ne" : "A2"}}, {"mr_delivery_type_code" : {"$ne" : "B1"}}, {"mr_delivery_type_code" : {"$ne" : "C1"}}, {"created_on":{"$gte":"'+this.state.filter_date.filter_list_date+' 00:00:00", "$lte":"'+this.state.filter_date.filter_list_date2+' 23:59:59"}}]');
    // filter_array.push('"$and" : [{"dsa_status.dsa_status_value" : "CREATED"}, {"dsa_status.dsa_status_date":{"$gte":"'+this.state.filter_date.filter_list_date+' 00:00:00", "$lte":"'+this.state.filter_date.filter_list_date2+' 23:59:59"}}]');
  ...
  async getAllMR() {
    ...
    // filter_array.push('"$and" : [{"dsa_status.dsa_status_value" : "CREATED"}, {"dsa_status.dsa_status_date":{"$gte":"'+this.state.filter_date.filter_list_date+' 00:00:00", "$lte":"'+this.state.filter_date.filter_list_date2+' 23:59:59"}}]');
    let whereAnd = '{' + filter_array.join(',') + '}';
    let res = await getDatafromAPINODE('/matreq?srt=_id:-1&noPg=1&q=' + whereAnd, this.state.tokenUser)
    if (res.data !== undefined) {
      const items = res.data.data;
  ...
  async downloadMRlist() {
    ...
    let headerRow = ["MR ID", "MR MITT ID","MR Type","MR Delivery Type", "Project Name", "CD ID", "Tower ID", "Tower Name", "Current Status", "Current Milestone", "DSP", "ETA", "MR MITT Created On", "MR MITT Created By","Updated On", "Created On", "MR Additional Reason", "MR Additional Remarks", "WP ID for WBS", "WBS HW", "WBS LCM", "WBS Katherin Antenna", "WBS LCM Antenna", "WBS LCM Bulk", "WBS LCM Power", "WBS HWAC (License)", "WBS PNRO", "WBS PNDO", "WBS Power System", "Cancellation Reason"];
    ws.addRow(headerRow);
    ...
    for (let i = 0; i < allMR.length; i++) {
      ...
      let mr_cancel_status = undefined;
      mr_cancel_status = allMR[i].mr_status.find(e => e.mr_status_name === "MATERIAL_REQUEST" && e.mr_status_value === "CANCELED");
      if(mr_cancel_status === undefined){
        mr_cancel_status = {};
      }
      ws.addRow([allMR[i].mr_id, allMR[i].mr_mitt_no, allMR[i].mr_type, allMR[i].mr_delivery_type, allMR[i].project_name, allMR[i].cust_del !== undefined ? allMR[i].cust_del.map(cd => cd.cd_id).join(', ') : allMR[i].cd_id, allMR[i].site_info[0] !== undefined ? allMR[i].site_info[0].site_id : null, allMR[i].site_info[0] !== undefined ? allMR[i].site_info[0].site_name : null, allMR[i].current_mr_status, allMR[i].current_milestones, allMR[i].dsp_company, convertDateFormatnewDate(allMR[i].eta), creator_mr_mitt !== undefined ? convertDateFormatnewDate(creator_mr_mitt.mr_status_date) : null, creator_mr_mitt !== undefined ? creator_mr_mitt.mr_status_updater : null, convertDateFormatnewDate(allMR[i].updated_on), convertDateFormatnewDate(allMR[i].created_on), allMR[i].mr_reason, allMR[i].mr_remark, dataCDIDIdx.WP_ID, dataCDIDIdx.C1003_WBS_HW, dataCDIDIdx.C1013_WBS_LCM, dataCDIDIdx.C1036_WBS_Kathrein, dataCDIDIdx.C1045_WBS_LCM_8107, dataCDIDIdx.C1033_WBS_LCM_Bulk, dataCDIDIdx.C1035_WBS_PowLCM_Site_Basis, dataCDIDIdx.C1008_WBS_HWAC, dataCDIDIdx.C1018_WBS_PNRO, dataCDIDIdx.C1024_WBS_PNDO, dataCDIDIdx.C01063_WBS_Power_2114, mr_cancel_status.mr_status_note]);
    ...
    saveAs(new Blob([allocexport]), 'MR Report '+this.state.filter_date.filter_list_date+' until '+this.state.filter_date.filter_list_date2+'.xlsx');
  }
  ...
  async downloadAllMRMigration() {
    let allMRList = [];
    let getMR = await getDatafromAPINODE('/matreq?noPg=1&srt=_id:-1&q={"mr_mitt_no" : {"$exists" : 1}, "mr_mitt_no" : {"$ne" : null}}', this.state.tokenUser);
            ...
            <Card>
              <CardHeader>
                ...
                <Dropdown size="sm" isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{ float: 'right', marginRight : '10px' }}>
                  ...
                  <DropdownMenu>
                    ...
                    {(this.state.userRole.findIndex(e => e === "BAM-Engineering") === -1 && this.state.userRole.findIndex(e => e === "BAM-Project Planner") === -1 && this.state.userRole.findIndex(e => e === "BAM-Warehouse") === -1 && this.state.userRole.findIndex(e => e === "BAM-LDM") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-Mover") === -1) && (
              ...
              <CardBody>
                <Row form>
                  <Col md={3}>
                    <FormGroup>
                      <Label>Start<span style={{color : 'rgba(216,67,21 ,1)'}}>*</span></Label>
                      <Input type="date" name="filter_list_date" value={this.state.filter_date.filter_list_date} onChange={this.handleFilterDate} />
                    </FormGroup>
                  </Col>
                  <Col md={3}>
                    <FormGroup>
                      <Label>End<span style={{color : 'rgba(216,67,21 ,1)'}}>*</span></Label>
                      <Input type="date" name="filter_list_date2" value={this.state.filter_date.filter_list_date2} onChange={this.handleFilterDate} />
                    </FormGroup>
                  </Col>
                </Row>
