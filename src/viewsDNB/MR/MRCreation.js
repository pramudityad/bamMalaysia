...
const MR_ADDITIONAL_REASONS = [
    { code:"R1", value: "CR" },
    { code:"R2", value: "Faulty" },
    { code:"R3", value: "Returned to WH/Access" },
    { code:"R4", value: "Mismatch Database" },
    { code:"R5", value: "Partial Delivery" },
    { code:"R6", value: "PICO Issue" },
    { code:"R7", value: "Stolen Issue" },
    { code:"R8", value: "LOM" },
    { code:"R9", value: "Interference" },
    { code:"R10", value: "Site Replacement" },
    { code:"R11", value: "Other Reason" }
];
...
class MRCreation extends Component {
  constructor(props) {
    ...
    this.loadOptionsMRID = this.loadOptionsMRID.bind(this);
    this.handleChangeMRID = this.handleChangeMRID.bind(this);
  ...
  async saveMRtoAPI(){
    ...
    if(dataValidation !== true){
      ...
      let list_site = [];
      let dataXLS = [];
      if(dataForm[3] === "Replacement"){
        dataXLS = [
          ["id", "project_name", "mr_type", "mr_delivery_type", "origin_warehouse", "etd", "eta", "deliver_by", "mr_comment_project", "sent_mr_request", "created_based", "identifier", "priority_mr", "week_number", "note_orn", "replacement_related"],
          ["new", this.state.project_name_selected, this.selectMRType(dataForm[3]), this.selectDeliveryType(dataForm[4]), dataForm[8], this.selectedDatetoFormat(dataForm[5]), this.selectedDatetoFormat(dataForm[6]), dataForm[7], "", "", this.state.identifier_by, this.state.tower_selected_id, dataForm[10], dataForm[11], dataForm[12], dataForm[13]]
        ]
      }else{
        dataXLS = [
      ...
      if(dataForm[3] === "Additional"){
        dataXLS = [
          ["id", "project_name", "mr_type", "mr_delivery_type", "origin_warehouse", "etd", "eta", "deliver_by", "mr_comment_project", "sent_mr_request", "created_based", "identifier", "priority_mr", "week_number", "mr_reason", "mr_remark"],
          ["new", this.state.project_name_selected, this.selectMRType(dataForm[3]), this.selectDeliveryType(dataForm[4]), dataForm[8], this.selectedDatetoFormat(dataForm[5]), this.selectedDatetoFormat(dataForm[6]), dataForm[7], "", "", this.state.identifier_by, this.state.tower_selected_id, dataForm[10], dataForm[11], dataForm[16], dataForm[17]]
        ];
      }
      if(this.selectDeliveryType(dataForm[4]) === "A1" || this.selectDeliveryType(dataForm[4]) === "A2" || this.selectDeliveryType(dataForm[4]) === "B1" || this.selectDeliveryType(dataForm[4]) === "C1" ){
        dataXLS = [
          ["id", "project_name", "mr_type", "mr_delivery_type", "origin_warehouse", "etd", "eta", "deliver_by", "mr_comment_project", "sent_mr_request", "created_based", "identifier", "priority_mr", "week_number", "destination_warehouse", "mr_remark"],
          ["new", this.state.project_name_selected, this.selectMRType(dataForm[3]), this.selectDeliveryType(dataForm[4]), dataForm[8], this.selectedDatetoFormat(dataForm[5]), this.selectedDatetoFormat(dataForm[6]), dataForm[7], "", "", this.state.identifier_by, this.state.tower_selected_id, dataForm[10], dataForm[11], dataForm[9], dataForm[17]]
        ]
      }
  ...
  async loadOptionsMRID(inputValue) {
    if(!inputValue) {
      return [];
    } else {
      let wp_id_list = [];
      const getWPID = await this.getDataFromAPINODE('/matreq?q={"mr_id":{"$regex":"'+inputValue+'", "$options":"i"}}&v={"mr_id" : 1}');
      if(getWPID !== undefined && getWPID.data !== undefined) {
        this.setState({list_cd_id : getWPID.data.data});
        wp_id_list.push({'value' : null , 'label' : null})
        getWPID.data.data.map(wp =>
          wp_id_list.push({'value' : wp._id , 'label' : wp.mr_id}))
      }
      return wp_id_list;
    }
  }
  ...
  showHide(e) {
    ...
    } else if(e.target.value === "Replacement") {
      this.setState({toggle_display : "replacement"});
    } else {
  ...
  selectDeliveryType(TypeDel){
    ...
    switch(TypeDel) {
      ...
      case "Site to Site":
        delType = 3;
        break;
      case "Port to Warehouse":
        delType = "A1";
        break;
      case "Warehouse to Port":
        delType = "A2";
        break;
      case "Warehouse to Warehouse":
        delType = "B1";
        break;
      case "Warehouse to Scrap":
        delType = "C1";
        break;
      default:
        delType = 1;
          ...
          <CardBody>
            <Form>
                ...
                <Col md={6}>
                  <FormGroup>
                    ...
                    <Input type="select" name="4" value={this.state.create_mr_form[4]} onChange={this.handleChangeFormMRCreation} style={this.state.validation_form.mr_delivery_type === false ? {borderColor : 'red'} : {}}>
                      ...
                      <option value="Site to Site" hidden={this.state.toggle_display !== "replacement"}>Site to Site</option>
              ...
              <Row form>
                ...
                {(this.state.create_mr_form[4] === "Warehouse to Warehouse" || this.state.create_mr_form[4] === "Warehouse to Port" || this.state.create_mr_form[4] === "Warehouse to Scrap" || this.state.create_mr_form[4] === "Port to Warehouse") && (
                  <Col md={6}>
                    <FormGroup>
              ...
              {this.state.create_mr_form[3] === "Additional" && (
              <Row form>
                <Col md={4}>
                  <FormGroup>
                    <Label>MR Reason</Label>
                    <Input type="select" name="16" value={this.state.create_mr_form[16]} onChange={this.handleChangeFormMRCreation}>
                      <option value="" disabled selected hidden>Select MR Reason</option>
                      {MR_ADDITIONAL_REASONS.map(mar =>
                        <option value={mar.code}>{mar.value}</option>
                      )}
                    </Input>
                  </FormGroup>
                </Col>
                <Col md={4}>
                  <FormGroup>
                    <Label>MR Remarks</Label>
                    <Input type="textarea" name="17" value={this.state.create_mr_form[17]} onChange={this.handleChangeFormMRCreation}></Input>
                  </FormGroup>
                </Col>
              </Row>
              )}
