...
class MRDetail extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      modal_cancel_mr : false,
      edit_mr_material : false,
    };
    ...
    this.toggleModalCancelMR = this.toggleModalCancelMR.bind(this);
  }
  ...
  toggleModalCancelMR(e) {
    this.setState((prevState) => ({
      modal_cancel_mr: !prevState.modal_cancel_mr
    }));
  }
  ...
  async getDataCDID(array_cd_id){
    ...
    let projection = '&projection={"WP_ID" : 1, "C1003_WBS_HW" : 1, "C1008_WBS_HWAC" : 1, "C1013_WBS_LCM" : 1, "C1018_WBS_PNRO" : 1, "C1024_WBS_PNDO" : 1, "C1032_WBS_HW_Bulk" : 1, "C1033_WBS_LCM_Bulk" : 1, "C1034_WBS_PowHW_Site_Basis" : 1, "C1035_WBS_PowLCM_Site_Basis" : 1, "C1036_WBS_Kathrein" : 1, "C1045_WBS_LCM_8107" : 1, "C01063_WBS_Power_2114" : 1}'
  ...
  prepareView() {
    ...
    if(mr_data.sow_type === "TRM"){
      let dataPPPS = Object.assign({}, data_pp);
  ...
  async downloadMaterialMRUpload() {
    ...
    const dataMR = this.state.data_mr;
    const dataItemMR = this.state.list_mr_item.filter(e => e.product_type.toLowerCase() !== "svc" && e.product_type.toLowerCase() !== "hwac" && e.product_type.toLowerCase() !== "sw");
    ...
    let dataCDIDIdx = {};
    if(dataMR.cust_del !== undefined && dataMR.cust_del[0] !== undefined){
      dataCDIDIdx = this.state.wbs_cd_id_data;
    ...
    for (let i = 0; i < dataItemMR.length; i++) {
      for (let j = 0; j < dataItemMR[i].materials.length; j++) {
        ...
        let dataWBS = null;
        if(dataMatIdx.material_origin !== undefined && dataMatIdx.material_origin !== null){
          if(dataMatIdx.material_origin.toLowerCase() === "eab"){
            dataWBS = dataCDIDIdx.C1003_WBS_HW;
          }
          if(dataMatIdx.material_origin.toLowerCase() === "lcm"){
            dataWBS = dataCDIDIdx.C1013_WBS_LCM;
          }
          if(dataMatIdx.material_origin.toLowerCase() === "eab antenna"){
            dataWBS = dataCDIDIdx.C1036_WBS_Kathrein;
          }
          if(dataMatIdx.material_origin.toLowerCase() === "lcm antenna"){
            dataWBS = dataCDIDIdx.C1045_WBS_LCM_8107;
            if(dataWBS === undefined || dataWBS === null || dataWBS.length === 0){
              dataWBS = dataCDIDIdx.C1013_WBS_LCM;
            }
          }
          if(dataMatIdx.material_origin.toLowerCase() === "eab power"){
            dataWBS = dataCDIDIdx.C01063_WBS_Power_2114;
          }
        }
        ws.addRow([
          dataMatIdx._id,
    ...
    for (let i = 0; i < dataItemMR.length; i++) {
      for (let j = 0; j < dataItemMR[i].materials.length; j++) {
        ...
        if(dataMatIdx.serial_numbers !== undefined && dataMatIdx.serial_numbers.length !== 0){
          let serial_number = dataMatIdx.serial_numbers.find(e => e.flag_name === "obd" || e.flag_name === "obd_web");
  ...
  async cancelMR(){
    this.toggleLoading();
    const dataMR = this.state.data_mr;
    this.patchDatatoAPINODE('/matreq/cancelMatreq/'+dataMR._id, {"cancelNote" : this.state.revision_note}).then(res => {
      if(res.data !== undefined){
        this.setState({ action_status : "success" });
        this.toggleLoading();
      }else{
    ...
    if(dataCDWBSbyCDID !== undefined){
      return (
        <Fragment>
          ...
          <td>{dataCDWBSbyCDID.C1045_WBS_LCM_8107}</td>
          ...
          <td>{dataCDWBSbyCDID.C01063_WBS_Power_2114}</td>
        </Fragment>
      )
    }else{
      return(
  ...
  getWBSMaterial(material_origin){
    let dataCDIDIdx = this.state.wbs_cd_id_data;
    if(dataCDIDIdx.length !== 0){
      let dataCDID_5 = dataCDIDIdx.find(dci => /_5$/.test(dci.WP_ID));
      if(dataCDID_5 === undefined){
        dataCDIDIdx = dataCDIDIdx[0];
    ...
    let dataWBS = null;
    if(material_origin !== undefined && material_origin !== null){
      if(material_origin.toLowerCase() === "eab"){
        dataWBS = dataCDIDIdx.C1003_WBS_HW;
      }
      if(material_origin.toLowerCase() === "lcm"){
        dataWBS = dataCDIDIdx.C1013_WBS_LCM;
      }
      if(material_origin.toLowerCase() === "eab antenna"){
        dataWBS = dataCDIDIdx.C1036_WBS_Kathrein;
      }
      if(material_origin.toLowerCase() === "lcm antenna"){
        dataWBS = dataCDIDIdx.C1045_WBS_LCM_8107;
        if(dataWBS === undefined || dataWBS === null || dataWBS.length === 0){
          dataWBS = dataCDIDIdx.C1013_WBS_LCM;
        }
      }
      if(material_origin.toLowerCase() === "eab power"){
        dataWBS = dataCDIDIdx.C01063_WBS_Power_2114;
      }
    }
    return dataWBS
  ...
  downloadMaterialAssingSerialNumber = async() => {
    const wb = new Excel.Workbook();
    const ws = wb.addWorksheet();
    const ws2 = wb.addWorksheet();
    ...
    let headerRow = ["site_id", "bundle_id", "bundle_name", "program", "site_title", "material_id", "material_name", "serial_number"];
    ws.addRow(headerRow);
    let list_material_id = [];
    for (let i = 0; i < dataItemMR.length; i++) {
      for (let j = 0; j < dataItemMR[i].materials.length; j++) {
        let dataMatIdx = dataItemMR[i].materials[j];
        ws.addRow([
          dataItemMR[i].site_id, dataItemMR[i].pp_id, dataItemMR[i].product_name, dataItemMR[i].program, dataItemMR[i].site_title, dataMatIdx.material_id, dataMatIdx.material_name,
        ]);
      }
    }

    const allocexport = await wb.xlsx.writeBuffer();
    saveAs( new Blob([allocexport]), "Material MR Serial Number" + dataMR.mr_id + " uploader.xlsx");
  }


  changeEditMR = () => {
    this.setState(prevState => ({ edit_mr_material: !prevState.edit_mr_material }));
  }

  saveUpdateMaterialSerialNumber = async() => {
    this.toggleLoading();
    const dataXLS = this.state.rowsXLS;
    const dataMR = this.state.data_mr;
    let patchDataMat = await this.patchDatatoAPINODE( "/matreq/uploadSerialNumber/" + dataMR._id, { data: dataXLS } );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Row>
          <Col xl="12">
            <Card>
              ...
              <CardBody>
                      ...
                      <Fragment>
                        ...
                        <div className="mr-detail__body--flex">
                          ...
                          {(this.state.data_mr.mr_delivery_type_code === "A1" || this.state.data_mr.mr_delivery_type_code === "A2" || this.state.data_mr.mr_delivery_type_code === "B1" ||this.state.data_mr.mr_delivery_type_code === "C1") && (
                            <div>
                              <div className="mr-detail__body--header-detail">
                                <span>MR Remarks</span>
                              </div>
                              <div>
                                {this.state.data_mr.mr_remark}
                              </div>
                            </div>
                          )}
                                      ...
                                      <Table responsive striped bordered size="sm">
                                        <thead>
                                          <tr style={{fontSize : '10.5px'}}>
                                            <th>WBS HW</th>
                                            <th>WBS LCM</th>
                                            <th>WBS Katherin Antenna</th>
                                            <th>WBS LCM Antenna</th>
                                            <th>WBS LCM Bulk</th>
                                            <th>WBS LCM Power</th>
                                            <th>WBS HWAC (License)</th>
                                            <th>WBS PNRO</th>
                                            <th>WBS PNDO</th>
                                            <th>WBS Power System</th>
                                          </tr>
                                        </thead>
                                        <tbody>
                                          <tr style={{fontSize : '10.5px'}}>
                ...
                {this.state.tabs_submenu[1] === true && (
                  <Fragment>
                              ...
                              {(this.state.userRole.findIndex(e => e === "BAM-Mover") === -1) && (this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASPWarehouse") === -1)  ? (
                                <Dropdown size="sm" isOpen={this.state.dropdownOpen[0]} toggle={() => {this.toggleDropdown(0);}} style={{float : 'left', marginRight : '10px'}}>
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownItem onClick={this.downloadMaterialAssingSerialNumber}> <i className="fa fa-file-text-o" aria-hidden="true"></i>Serial Number Format Update</DropdownItem>
                                  </DropdownMenu>
                                </Dropdown>
                              ) : (
                                ...
                                {this.state.data_mr !== null && (this.state.data_mr.mr_delivery_type_code !== "A1" && this.state.data_mr.mr_delivery_type_code !== "A2" && this.state.data_mr.mr_delivery_type_code !== "B1" && this.state.data_mr.mr_delivery_type_code !== "C1") && (
                                <td>
                                {this.state.mr_site_FE !== null &&
                                  this.state.data_mr.mr_type !== "Relocation" &&
                                  this.state.data_mr.mr_type !== "Return" ? (
                                    <Fragment>
                                      <Button onClick={this.changeEditMR} size="sm" color="info">Edit</Button>
                                    </Fragment>
                                  ) : (
                                    <Fragment>
                                      <Button onClick={this.changeEditMR} size="sm" color="info">Edit</Button>
                    ...
                    {(this.state.edit_mr_material === true) && (
                    <Row style={{marginTop : '20px'}}>
                      <Col>
                        <Card>
                        <CardBody>
                        {((this.state.userRole.findIndex(e => e === "BAM-Mover") === -1) && (this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASPWarehouse") === -1) && status_can_edit_material.includes(this.state.data_mr.current_mr_status)) && (
                          <Row>
                            <Col  md="6">
                              <div><span>Change to Material Variant :</span></div>
                              <div>
                              <input type="file" onChange={this.fileHandlerMaterial.bind( this )} style={{ visiblity: "hidden" }} />
                              <Button size="sm" color="success" style={{ float: "right" }} onClick={this.saveUpdateMaterial}
                            ...
                            {/*}<Col  md="6">
                              <div><span>Update CPO Number :</span></div>
                              <div>
                              <input type="file" onChange={this.fileHandlerMaterialCPO.bind( this )} style={{ visiblity: "hidden" }} />
                              <Button size="sm" color="success" style={{ float: "right" }} onClick={this.saveUpdateMaterialCPO}
                                disabled={ this.state.rowsXLSCPO.length === 0 } >
                                Update CPO
                              </Button>
                              </div>
                            </Col> */}
                        ...
                        {((this.state.userRole.findIndex(e => e === "BAM-Mover") === -1) && (this.state.userRole.findIndex(e => e === "BAM-ASP Management") === -1 && this.state.userRole.findIndex(e => e === "BAM-ASP") === -1)) && (
                          <Row style={{marginTop : '15px'}}>
                            <Col  md="6">
                              <div><span>Adding Serial Number :</span></div>
                              <div>
                              <input type="file" onChange={this.fileHandlerMaterial.bind( this )} style={{ visiblity: "hidden" }} />
                              <Button size="sm" color="success" style={{ float: "right" }} onClick={this.saveUpdateMaterialSerialNumber}
                                disabled={ this.state.rowsXLS.length === 0 } >
                                Save
                              </Button>
                              </div>
                    ...
                    <div>PlantSpec Group No : {this.state.data_mr !== null ? (<Link to={'/ps-detail/'+this.state.data_mr.id_plantspec_doc}>{this.state.data_mr.no_plantspec}</Link>) : ""}</div>
                    {this.state.data_mr !== null && (this.state.data_mr.mr_delivery_type_code === "A1" || this.state.data_mr.mr_delivery_type_code !== "A2" && this.state.data_mr.mr_delivery_type_code !== "B1" && this.state.data_mr.mr_delivery_type_code !== "C1" ) ? (
                      ...
                      {(this.state.data_mr !== null && this.state.data_mr.sow_type !== "TRM") ? (
                        <Table hover bordered striped responsive size="sm">
                          <thead style={{ backgroundColor: "#0B486B", color: "white" }} className="table-mr__header--fixed" >
                            <tr>
                              ...
                              <th rowSpan="2" className="fixedhead" style={{verticalAlign: "middle" }}>
                                PS No. / Material Source (WBS Number)
                              </th>
                            </tr>
                            {this.state.data_mr !== null &&
                            this.state.data_mr.mr_type !== "Relocation" &&
                                  ...
                                  <Fragment>
                                    ...
                                    {pp.materials.map((material) => (
                                      ...
                                      >
                                        ...
                                        <td>{material.source_material} {this.getWBSMaterial(material.material_origin) !== null ? "(WBS : "+this.getWBSMaterial(material.material_origin) +")" : null }</td>
                                      </tr>
                                    ))}
                                  </Fragment>
                                ))}
                        ...
                        <Table hover bordered striped responsive size="sm">
                          ...
                          {this.state.data_mr !== null && status_can_edit_material.includes(this.state.data_mr.current_mr_status) ? (
                            <tbody>
                              {this.state.mr_site_NE !== null && this.state.list_mr_item.filter(e => e.product_type.toLowerCase() !== "svc").map((pp) => (
                                  <Fragment>
                            ...
                            <tbody>
                              {this.state.mr_site_NE !== null && this.state.list_mr_item.map((pp) => (
                                  <Fragment>
                                    <tr style={{ backgroundColor: "#E5FCC2" }} className="fixbody" >
                                      <td style={{ textAlign: "left" }}>
                                        {pp.pp_id}
                        ...
                        <Table hover bordered striped responsive size="sm">
                          <thead style={{ backgroundColor: "#0B486B", color: "white" }} className="table-mr__header--fixed">
                            <tr>
                              ...
                              <th>PP / Material Name</th>
                              <th>Unit</th>
                              <th>Qty</th>
                            </tr>
                          </thead>
                          <tbody>
                            {this.state.data_mr !== null && this.state.data_mr.packages.map(mp =>
                              <Fragment>
                              <tr style={{backgroundColor : "rgba(78,205,196,1)"}}>
                                <td>{mp.pp_id}</td>
                                <td>{mp.product_name}</td>
                                <td>{mp.uom}</td>
                                <td>{mp.qty}</td>
                              </tr>
                              {mp.materials.map(mpm =>
                                <tr style={{backgroundColor:"rgba(248,246,223, 0.5)"}}>
                                  <td style={{textAlign : 'right'}}>{mpm.material_id}</td>
                                  <td style={{textAlign : 'left'}}>{mpm.material_name}</td>
                                  <td>{mpm.uom}</td>
                                  <td>{mpm.qty}</td>
                                </tr>
                              )}
              ...
              {(this.state.data_mr !== null) && (
              <CardFooter>
              {(this.state.userRole.findIndex(e => e === "BAM-LDM") !== -1 || this.state.userRole.findIndex(e => e === "Admin") !== -1) && this.state.data_mr.mr_status.find(ms => ms.mr_status_name === "LOADING_PROCESS" && ms.mr_status_name === "FINISH") === undefined ? (
                <Button
                  color="warning"
                  style={{ float: "left" }}
                  size="sm"
              ...
              {(this.state.userRole.findIndex(e => e === "BAM-LDM") !== -1 || this.state.userRole.findIndex(e => e === "Admin") !== -1) && this.state.data_mr !== null && this.state.data_mr.current_mr_status === "MR REQUESTED" ? (
                    ...
                    <div>
                      ...
                      {(this.state.userRole.findIndex(e => e === "BAM-Engineering") === -1 && this.state.data_mr.mr_status.findIndex(st => st.mr_status_name === "LOADING_PROCESS" && st.mr_status_value === "FINISH") === -1) && (
                        <Button
                          ...
                          size="sm"
                          onClick={this.toggleModalCancelMR}
                        >
                          Cancel MR
                        </Button>
                      )}
        ...
        <Modal isOpen={this.state.modal_cancel_mr} toggle={this.toggleModalCancelMR} className={"modal-sm"}>
          <ModalHeader>Note for Cancel</ModalHeader>
          <ModalBody>
            <Input type="textarea" rows="9" placeholder="Note..." onChange={this.handleRevisionNote} value={this.state.revision_note} />
          </ModalBody>
          <ModalFooter>
            <Button color="danger" style={{ float: "right", marginRight: "8px" }} onClick={this.cancelMR}><i className="fa fa-edit" style={{ marginRight: "8px" }}></i> Cancel MR</Button>
            <Button color="secondary" onClick={this.toggleModalCancelMR}>Cancel</Button>
          </ModalFooter>
        </Modal>
