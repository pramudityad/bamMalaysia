...
class TSSRBoq extends Component {
    constructor(props) {
      ...
      this.onChangeFilterDetailTSSR = debounce(this.onChangeFilterDetailTSSR, 500);
    }
    ...
    exportFormatTSSRHorizontalWithDRM = async () =>{
      ...
      for(let i = 0; i < dataSites.length ; i++){
        ...
        ws.addRow([dataSites[i].site_id, dataSites[i].program, dataSites[i].sow, dataSites[i].sow_code, dataSites[i].priority]
          .concat(qtyConfig)
    ...
    onChangeFilterDetailTSSR = () => {
      let data_tssr_boq_filter = JSON.parse(JSON.stringify(this.state.data_tssr_boq_sites));
      if(this.state.filter_detail_tssr.tower_id !== undefined){
        const regex = new RegExp(this.state.filter_detail_tssr.tower_id, 'gi');
        data_tssr_boq_filter = data_tssr_boq_filter.filter((bf) => bf.site_id.match(regex));
      }else{
      ...
      if(this.state.filter_detail_tssr.program !== undefined){
        ...
        data_tssr_boq_filter = data_tssr_boq_filter.filter((bf) => bf.program.match(regex));
      }else{
        data_tssr_boq_filter = data_tssr_boq_filter;
      }
      if(this.state.filter_detail_tssr.config_id !== undefined){
        ...
        for(let j = 0; j < data_tssr_boq_filter.length; j++){
          data_tssr_boq_filter[j]["siteItemConfig"] = data_tssr_boq_filter[j].siteItemConfig.filter((bf) => bf.config_id.match(regex));
        }
      }else{
        data_tssr_boq_filter = data_tssr_boq_filter;
      }
                  ...
                  {this.state.view_drm === "tssr" && (
                    <div class='divtable'>
                      <Table hover bordered striped responsive size="sm" width="100%">
                          ...
                          <tr>
                            <th><input onChange={this.handleFilterListDetailTSSR} name="tower_id"/></th>
                            <th><input onChange={this.handleFilterListDetailTSSR} name="program"/></th>
                            <th><input onChange={this.handleFilterListDetailTSSR} name="config_id"/></th>
                          </tr>
                        </thead>
                        <tbody>
                        {this.state.version_selected !== null && this.state.data_tssr_boq.version !== this.state.version_selected ? (
