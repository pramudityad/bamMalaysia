...
class TssrList extends Component {
  ...
  getTssrList() {
    ...
    this.state.filter_list["cd_id"] !== null &&
      this.state.filter_list["cd_id"] !== undefined &&
      filter_array.push(
        '"site_info.cd_id":{"$regex" : "' +
          this.state.filter_list["cd_id"] +
          '", "$options" : "i"}'
      );
    this.state.filter_list["region"] !== null &&
      this.state.filter_list["region"] !== undefined &&
      filter_array.push(
        '"site_info.region":{"$regex" : "' +
    ...
    this.state.filter_list["submission_status"] !== null &&
      this.state.filter_list["submission_status"] !== undefined &&
      filter_array.push(
        '"submission_status":{"$regex" : "' +
          this.state.filter_list["submission_status"] +
          '", "$options" : "i"}'
      );
    ...
    this.getDataFromAPINODE(
      ...
      if (res.data !== undefined) {
        ...
        res.data.data.map((lst) =>
          lst.site_info.map((st) => siteIDArray.push(st.site_id))
        );
  ...
  async getTssrListMigration() {
    ...
    // srt=_id:-1&
    // + '&lmt=' + maxPage + '&pg=' + page
    let dataMigrationTSSR = await this.getDataFromAPINODE(
      "/plantspec?noPg=1&q=" + whereAnd + '&v={"_id" : 1}'
    );
  ...
  async printExcel() {
    ...
    for (let a = 0; a < dataTSSRMigration.length; a++) {
      const ps = await this.getDataFromAPINODE(
        "/plantspec/" + dataTSSRMigration[a]._id
      );
  ...
  async getPSListReport() {
    ...
    // + '&lmt=' + maxPage + '&pg=' + page
    let dataMigrationTSSR = await this.getDataFromAPINODE(
      "/plantspec?noPg=1&srt=_id:-1&q=" + whereAnd
    );
    if (dataMigrationTSSR.data !== undefined) {
      arrayIDTSSR = dataMigrationTSSR.data.data;
      return arrayIDTSSR;
  ...
  async printExcelListPS() {
    ...
    for (let i = 0; i < dataTSSRMigration.length; i++) {
      ...
      if (dataTSSRMigration[i].plantspec_status !== undefined) {
        CreatedBy = dataTSSRMigration[i].plantspec_status.find(
          (pst) => pst.status_value === "CREATED"
        );
      ...
      if (
        dataTSSRMigration[i].site_info !== undefined &&
        dataTSSRMigration[i].site_info.length !== 0
      ) {
        dataSite = dataTSSRMigration[i].site_info[0];
      }
      ws.addRow([
        dataTSSRMigration[i].no_plantspec,
        dataTSSRMigration[i].project_name,
        dataSite.site_id,
        dataSite.site_name,
        dataSite.region,
        dataTSSRMigration[i].current_status,
        dataTSSRMigration[i].submission_status,
        dataTSSRMigration[i].mr_id,
        CreatedBy,
      ]);
    }
    const allocexport = await wb.xlsx.writeBuffer();
  ...
  getDataTower(array_tower_id) {
    ...
    this.getDatafromAPIXL(
      '/tower_site_op?where={"tower_id" :{"$in" : [' +
        array_tower +
                          ...
                          <React.Fragment>
                            ...
                            <td>
                              {list.site_info !== undefined
                                ? list.site_info.map((si) => si.cd_id)
                                : null}
                            </td>
