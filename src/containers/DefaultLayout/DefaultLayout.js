...
class DefaultLayout extends Component {
  ...
  showMenuByRole() {
    ...
    let dataMenuRoles = [];
    if (
      this.state.vendor_code !== undefined &&
      this.state.vendor_code !== null &&
      this.state.vendor_code.length !== 0
    ) {
      ...
      for (let i = 0; i < dataMenu.length; i++) {
        ...
        if (dataMenu[i].roles !== undefined) {
          ...
          if (allowed === false) {
            ...
            if (
              dataMenu[i].children !== undefined &&
              dataMenu[i].children.length > 0
            ) {
              for (let j = 0; j < dataMenu[i].children.length; j++) {
                if (dataMenu[i].children[j].roles !== undefined) {
                  let allowedChild = dataMenu[i].children[j].roles.some((e) =>
                    rolesUser.includes(e)
                  );
                  if (allowedChild === false) {
                    // dataMenuIndex.children.splice(j,1);
                    dataMenuIndex.children = dataMenuIndex.children.filter(
                      (e) => e.name !== dataMenu[i].children[j].name
                    );
                  }
                }
              }
