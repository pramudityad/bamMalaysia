...
class HWEdit extends React.PureComponent {
  ...
  getListID(_id) {
    getDatafromAPINODE(
      "/cpoMapping/getCpo/required/hw/" + _id,
  ...
  saveUpdate = async () => {
    ...
    const allowed_header =
      this.state.roleUser.includes("BAM-MAT PLANNER") === true
        ...
        ? global.config.cpo_mapping.hw.header_pfm
        : global.config.cpo_mapping.hw.header_admin;
    const header_post =
      this.state.roleUser.includes("BAM-MAT PLANNER") === true
        ? global.config.cpo_mapping.hw.header_materialmapping
        : this.state.roleUser.includes("BAM-PFM") === true
        ? global.config.cpo_mapping.hw.header_pfm.concat(
            "Reference_Loc_Id",
            "Po",
            "Line",
            "Qty"
          )
        : global.config.cpo_mapping.hw.header_admin.concat(
            "Reference_Loc_Id",
            "Po",
            "Line",
            "Qty"
