...
import "./LMRMY.css";
import { getDatafromAPINODE, postDatatoAPINODE, generateTokenACT, getDatafromAPIMY, sendEmailNotification } from "../../helper/asyncFunctionDigiSPDH";
...
class MYASGCreation extends Component {
  ...
  getDataCDACT_Fas() {
    this.toggleLoading();
    this.getFASfromACT("https://dev-corsanywhere.e-dpm.com/", "http://10.0.0.35:80/api/get_data_auth").then((resCD) => {
  ...
  async createLMR() {
    ...
    const dataLMR = {
      ...
      lmr_approval_status: 'Waiting for IM Approval',
    ...
    if (empty_nw) {
      ...
      const respondSaveLMR = await postDatatoAPINODE("/aspassignment/saveOneAspAssignment", { asp_data: dataLMR, asp_data_child: dataLMRChild }, this.state.tokenUser);
      if (respondSaveLMR.data !== undefined && respondSaveLMR.status >= 200 && respondSaveLMR.status <= 300) {
        localStorage.removeItem("asp_data");
        localStorage.removeItem("asp_data_child");

        const bodyEmail = `Hi IM,<br/>Please approve this LMR.<br/><br/>${process.env.REACT_APP_BASE_URL}/lmr-edit/${respondSaveLMR.data.parent._id}<br/><br/>Thank you.<br/>ASP Team`;
        const emailContent = {
          "to": "a.fariz.mursyidan@ericsson.com",
          "subject": "LMR Approval",
          "body": bodyEmail
        }
        const sendEmail = await sendEmailNotification(emailContent);
